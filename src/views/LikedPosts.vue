<template>
    <div class="min-h-screen bg-gradient-to-br from-purple-50 via-pink-50 to-blue-50 darkVitals:from-gray-900 dark:via-purple-900 dark:to-blue-900 py-8 px-4 sm:px-6 lg:px-8">
      <div class="max-w-7xl mx-auto pb-20">
        ...
        <div class="flex justify-between items-center mb-8">
          <h1 class="text-4xl font-bold bg-gradient-to-r from-purple-600 to-indigo-500 bg-clip-text text-transparent animate-gradient-text flex items-center">
            <i class="fas fa-heart mr-3 text-purple-600 dark:text-purple-400"></i>
        ...
        <div v-if="isLoading" class="flex flex-col items-center justify-center min-h-[400px] animate-fade-in">
          ...
          <p class="mt-4 text-lg text-gray-600 dark:text-gray-400 animate-pulse">
            Загрузка самых понравившихся постов...
          </p>
        </div>
  
        <!-- Список постов -->
        <div v-else-if="likedPosts.length > 0" class="space-y-6">
          <div v-for="post in likedPosts" :key="post.id" class="group">
  ...
  // Получение самых понравившихся постов, отсортированных по likesCount
  const likedPosts = computed(() => {
    const posts = Object.values(store.state.posts.posts || {});
    console.log('Все посты из store:', posts); // Отладка
    
    const mappedPosts = posts.map(post => {
      const likesCount = post.likes ? Object.keys(post.likes).length : 0; // Подсчитываем likesCount
    ...
    console.log('Обработанные посты:', mappedPosts); // Отладка
    
    return mappedPosts
      .filter(post => post.likesCount !== undefined) // Убеждаемся, что likesCount есть
      .sort((a, b) => (b.likesCount || 0) - (a.likesCount || 0)); // Сортировка по убыванию likesCount
  });

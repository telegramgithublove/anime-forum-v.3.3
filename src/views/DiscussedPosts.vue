...
onMounted(async () => {
  console.log('CategoryPosts.vue: Монтирование компонента для categoryId:', categoryId);
  try {
    ...
    if (categorySnapshot.exists()) {
      ...
      console.log('CategoryPosts.vue: Загружена категория:', { name: categoryName.value, description: categoryDescription.value });
      ...
      unsubscribe = onValue(postsRef, async (snapshot) => {
        ...
        if (snapshot.exists()) {
          ...
          const categoryPostIds = categoryData.postIds ? Object.keys(categoryData.postIds) : [];
          
          const postsArray = await Promise.all(categoryPostIds.map(async (postId) => {
            ...
            if (post.authorId && post.authorId !== currentUserId) {
              try {
                ...
                console.error('[CategoryPosts] Ошибка загрузки профиля:', error);
              }
          ...
          posts.value = postsArray.filter(post => post !== null);
          store.dispatch('pagination/setTotalItems', posts.value.length);
          store.dispatch('pagination/setItemsPerPage', itemsPerPage);
          console.log('CategoryPosts.vue: Загружены посты:', posts.value);
        ...
        console.error('CategoryPosts.vue: Ошибка в подписке onValue:', error);
        isLoading.value = false;
        loadingProgress.value = 0;
      });
    } else {
    ...
    console.error('CategoryPosts.vue: Ошибка при загрузке данных:', error);
    isLoading.value = false;
    loadingProgress.value = 0;
  }
});
...
const toggleLike = async (postId) => {
  ...
  try {
    ...
    if (!postSnapshot.exists()) throw new Error('Пост не найден');

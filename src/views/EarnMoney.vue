    ...
    <div class="min-h-screen bg-gradient-to-br from-purple-900 via-indigo-800 to-blue-900 py-12 px-4">
          ...
          <div class="relative z-20 p-6 h-full flex flex-col justify-between">
            ...
            <div class="space-y-4">
              <p class="text-white font-bold text-lg">
                Стоимость: <span class="text-yellow-400">{{ card.cost }}</span> Preycoin
              </p>
              <button 
                ...
                class="w-full py-4 px-8 rounded-lg font-bold text-white text-lg transition-all duration-300 transform
                       ...
                       active:scale-95">
                {{ userBalance >= card.requiredBalance ? 'Активировать' : 'Недостаточно Preycoin' }}
              </button>
            </div>
          </div>
      ...
      <div class="max-w-4xl mx-auto mt-16 mb-20 p-8 bg-gray-900/50 backdrop-blur-md rounded-xl shadow-2xl text-white">
        <h2 class="text-3xl font-bold text-center mb-6 font-anime text-yellow-400 animate-float">
          Как заработать Preycoin?
        </h2>
        <p class="text-lg mb-4">
          <span class="text-pink-400 font-bold">Preycoin</span> — это валюта нашего портала. Один Preycoin = одному российскому рублю.
        </p>
        <p class="mb-4">
          В профиле вашего аккаунта есть <span class="text-yellow-400">Progress Bar</span>. Зайдите туда и посмотрите. Вы начинаете зарабатывать Preycoin с вашего первого статуса <span class="text-pink-400">New User</span>. Каждый ваш пост в любой категории форума стоит <span class="text-yellow-400">1 Preycoin</span>.
        </p>
        <p class="mb-4">
          Первый шаг — самая сложная часть пути. Когда вы набираете <span class="text-yellow-400">200 Preycoin</span>, вы можете активировать свою первую карточку и получить небольшие бонусы. Дальше будет легче!
        </p>
        <ul class="space-y-4 text-base">
          <li class="flex items-start gap-2">
            <star-icon class="w-5 h-5 text-pink-400 flex-shrink-0" />
            <span>Статус <span class="text-pink-400">User</span>: до второй карточки вам нужно набрать <span class="text-yellow-400">700 Preycoin</span>. Теперь у вас есть доступ к уникальным темам, где каждая тема стоит <span class="text-yellow-400">20 Preycoin</span>, а обычная — <span class="text-yellow-400">10 Preycoin</span>.</span>
          </li>
          <li class="flex items-start gap-2">
            <star-icon class="w-5 h-5 text-pink-400 flex-shrink-0" />
            <span>Статус <span class="text-pink-400">Moderator</span> (<span class="text-yellow-400">1000 Preycoin</span>): вы получаете бонусы модератора. Обычные посты стоят <span class="text-yellow-400">20 Preycoin</span>, а в уникальных категориях — <span class="text-yellow-400">30 Preycoin</span>.</span>
          </li>
          <li class="flex items-start gap-2">
            <star-icon class="w-5 h-5 text-pink-400 flex-shrink-0" />
            <span>Статус <span class="text-pink-400">Teacher</span> (<span class="text-yellow-400">1000 Preycoin</span>): вы получаете бонусы + <span class="text-yellow-400">5000 рублей</span> за 12 ч/месяц помощи администрации. Гибкий график по вашему календарю.</span>
          </li>
          <li class="flex items-start gap-2">
            <star-icon class="w-5 h-5 text-pink-400 flex-shrink-0" />
            <span>Статус <span class="text-pink-400">Administrator</span> (<span class="text-yellow-400">1700 Preycoin</span>): вы получаете бонусы + <span class="text-yellow-400">15,000 рублей</span> за 150 мин/месяц помощи команде разработчиков. Гибкий график, работа 25 минут в неделю в админ-панели.</span>
          </li>
        </ul>
        <p class="mt-6">
      ...
      <div class="fixed top-4 right-4 bg-gray-800/80 backdrop-blur-sm p-8 rounded-lg text-white">
        Ваш баланс: <span class="text-yellow-400 font-bold">{{ userBalance }}</span> Preycoin
      </div>
    </div>
</template>
...
const userBalance = computed(() => store.getters['userBalance'])
...
const cards = ref([
  {
    ...
    benefits: [
      ...
      'Лимит символов увеличен до 666'
    ]
  },
  {
    title: 'Moderator',
    ...
    benefits: [
      ...
      'Обычные посты: 10 Preycoin',
      'Уникальные темы: 25 Preycoin',
      'Вывод 500 Preycoin на карту'
    ]
  },
  {
    title: 'Teacher',
